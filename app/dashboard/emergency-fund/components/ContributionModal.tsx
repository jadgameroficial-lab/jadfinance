...
import type {
  EmergencyFundRow,
  EmergencyFundContributionRow,
  EmergencyFundContributionInsert,
  EmergencyFundContributionUpdate,
} from "@/services/emergencyFund.service";
import { useToast } from "@/lib/toast";
import { ModalShell } from "./EmergencyFundFormModal";
...
export function ContributionModal({
  ...
  /** Reserva para a qual o modal está aberto; null = modal fechado. */
  ...
  const [editingContribution, setEditingContribution] =
    useState<EmergencyFundContributionRow | null>(null);
  const [amount, setAmount] = useState("");
  const [date, setDate] = useState(todayIso());
  const [note, setNote] = useState("");
  const [saving, setSaving] = useState(false);
  const [formError, setFormError] = useState<string | null>(null);
  const [pendingDelete, setPendingDelete] = useState<EmergencyFundContributionRow | null>(null);
  ...
  function startEdit(contribution: EmergencyFundContributionRow) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      if (editingContribution) {
        await updateContribution(editingContribution.id, fund.id, {
          amount: parsedAmount,
  ...
  async function handleConfirmDelete() {
    if (!pendingDelete || !fund) return;
    setDeleting(true);
    try {
      await deleteContribution(pendingDelete.id, fund.id);
  ...
  return (
    <>
      <ModalShell
        open={open}
        title={fund ? `Aportes — ${fund.name}` : "Aportes"}
      ...
      <ConfirmDialog
        ...
        message="Tem certeza que deseja excluir este aporte? O valor da reserva será recalculado automaticamente. Esta ação não poderá ser desfeita."
