...
const DEFAULT_ICON = "shield-check";

/**
 * Ícones disponíveis para uma reserva de emergência. Chave é o que fica
 * salvo em emergency_funds.icon (texto livre no banco); o mapa serve tanto
 * para o seletor aqui quanto para o EmergencyFundCard resolver o ícone de
 * volta a um componente visual.
 */
export const EMERGENCY_FUND_ICON_MAP: Record<string, LucideIcon> = {
...
const PRIORITIES: { value: EmergencyFundRow["priority"]; label: string }[] = [
 ...
 * módulo Reserva de Emergência (EmergencyFundFormModal e ContributionModal),
 * para não duplicar essa estrutura entre os dois.
 */
...
export function EmergencyFundFormModal({
  ...
  const [priority, setPriority] = useState<EmergencyFundRow["priority"]>("media");
  ...
  useEffect(() => {
    ...
    if (editing) {
      ...
      setMonthlyExpense(String(editing.monthly_expense).replace(".", ","));
      setMonthsTarget(String(editing.months_target));
    } else {
      setName("");
      setDescription("");
  ...
  const parsedMonthlyExpense = parseAmount(monthlyExpense);
  const parsedMonthsTarget = parseMonths(monthsTarget);
  const previewTarget =
    parsedMonthlyExpense !== null && parsedMonthlyExpense > 0 && parsedMonthsTarget !== null
      ? parsedMonthlyExpense * parsedMonthsTarget
      : null;
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!trimmedName) {
      setFormError("Informe um nome para a reserva.");
      return;
    }

    if (parsedMonthlyExpense === null || parsedMonthlyExpense <= 0) {
      setFormError("Informe uma despesa mensal válida, maior que zero.");
      return;
    }

    if (parsedMonthsTarget === null) {
      setFormError("Informe uma quantidade de meses válida, maior que zero.");
      return;
    }
    ...
    try {
      const payload = {
        ...
        months_target: parsedMonthsTarget,
      };
    ...
    >
      <form onSubmit={handleSubmit}>
        ...
        <p style={{ fontSize: 11.5, color: "var(--text-ter)", margin: "-6px 0 18px", lineHeight: 1.5 }}>
          O valor objetivo é calculado automaticamente pelo banco: despesa mensal × quantidade de
          meses{previewTarget !== null ? ` — atualmente R$ ${formatPreview(previewTarget)}` : ""}.
        </p>
        ...
        <div className="dash-d-field">
          ...
          <div className="dash-icon-picker">
            {Object.entries(EMERGENCY_FUND_ICON_MAP).map(([key, Icon]) => (
              <button
                key={key}
                type="button"
        ...
        <p style={{ fontSize: 11.5, color: "var(--text-ter)", margin: "-6px 0 18px", lineHeight: 1.5 }}>
          ...
          registrados nesta reserva — use &quot;Adicionar aporte&quot; no card para lançar valores.
        </p>
