...
import type { GoalContributionRow } from "@/services/goals.service";
...
/**
 * Histórico de aportes de uma meta — mais recente primeiro (a própria query
 * do service já entrega nessa ordem). Só exibe/dispara ações; quem decide o
 * que fazer com editar/excluir é o ContributionModal, que é quem a monta.
 */
export function ContributionHistory({
  ...
  busyId: string | null;
  onEdit: (contribution: GoalContributionRow) => void;
  onDelete: (contribution: GoalContributionRow) => void;
}) {
  ...
  if (contributions.length === 0) {
    return (
      <p style={{ fontSize: 12.5, color: "var(--text-ter)", padding: "10px 0" }}>
        Nenhum aporte registrado ainda para esta meta.
      </p>
    );
  }
